...
{
    internal sealed class CloseCustomerCommandHandler(ICustomerRepository repository)
        : IRequestHandler<CloseCustomerCommand>
    {
        public async Task Handle(CloseCustomerCommand command, CancellationToken cancellationToken)
        {
            ...
            customer.CloseCustomer(command);
