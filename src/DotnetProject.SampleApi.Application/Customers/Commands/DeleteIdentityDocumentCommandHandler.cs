...
{
    internal sealed class DeleteIdentityDocumentCommandHandler(ICustomerRepository repository)
        : IRequestHandler<DeleteIdentityDocumentCommand>
    {
        public async Task Handle(DeleteIdentityDocumentCommand command, CancellationToken cancellationToken)
        {
            ...
            customer.DeleteIdentityDocument(command);
