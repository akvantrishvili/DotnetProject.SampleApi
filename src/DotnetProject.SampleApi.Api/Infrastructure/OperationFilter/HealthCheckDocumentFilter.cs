...
namespace DotnetProject.SampleApi.Api.Infrastructure.OperationFilter
{
    public class HealthCheckDocumentFilter : IDocumentFilter
    {
        ...
        {
            this._routePath = !string.IsNullOrEmpty(routePath) ? routePath : throw new ArgumentNullException(nameof(routePath));
                ...
                dictionary = (IDictionary<HealthStatus, int>)new Dictionary<HealthStatus, int>()
        {
          ...
          },
          {
            ...
            this._resultStatusCodes = dictionary;
        }
        ...
        public void Apply(OpenApiDocument swaggerDoc, DocumentFilterContext context)
        {
            ...
            operation.Description = "Check if service is " + string.Join<HealthStatus>(", ", (IEnumerable<HealthStatus>)this._resultStatusCodes.Keys);
            operation.Tags.Add(new OpenApiTag()
            {
                Name = "Health Check"
            });
            foreach (KeyValuePair<string, IEnumerable<string>> keyValuePair in this._resultStatusCodes.GroupBy<KeyValuePair<HealthStatus, int>, int>((Func<KeyValuePair<HealthStatus, int>, int>)(x => x.Value)).ToDictionary<IGrouping<int, KeyValuePair<HealthStatus, int>>, string, IEnumerable<string>>((Func<IGrouping<int, KeyValuePair<HealthStatus, int>>, string>)(x => x.Key.ToString()), (Func<IGrouping<int, KeyValuePair<HealthStatus, int>>, IEnumerable<string>>)(x => x.Select<KeyValuePair<HealthStatus, int>, string>((Func<KeyValuePair<HealthStatus, int>, string>)(y => y.Key.ToString())))))
            {
